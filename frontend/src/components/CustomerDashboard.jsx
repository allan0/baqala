...
export default function CustomerDashboard({
  ...
  useEffect(() => {
    const fetchBaqalas = async () => {
      try {
        ...
        console.error("Failed to fetch nearby baqalas:", err);
        setNearbyBaqalas([]);
      }
    };
    fetchBaqalas();
  ...
  const checkoutCart = async () => {
    ...
    try {
      ...
      setCurrentScreen('hisaab'); // Switch to Hisaab tab after purchase
        ...
        <div className="inventory-grid">
          {selectedBaqala.inventory.length === 0 ? (
            ...
            <div key={item.id} className="card item-card">
              ...
              <p className="crypto-price">AED {(item.price * (1 - (item.cryptoDiscount || 10) / 100)).toFixed(2)}</p>
        ...
        <>
          <div className="hisaab-card" style={{ marginBottom: '25px' }}>
            ...
            <button className="btn-primary" onClick={handleSettle} disabled={cashTotal === 0}>
              {cashTotal === 0 ? "✅ All Settled" : walletAddress ? "💳 Pay with Crypto" : "🔐 Connect Wallet"}
            </button>
          </div>
          ...
          <div className="card">
            ...
            <ul className="order-list">
              {activeProfile.unpaidItems.length === 0 ? (
                <p style={{ color: 'var(--lux-hint)', textAlign: 'center', padding: '30px 0' }}>
                  No pending items on this profile 🎉
                </p>
              ) : (
                activeProfile.unpaidItems.map((item, i) => (
                  <li key={i}>
                    <div>
                      ...
                      <small className="badge-crypto">{item.cryptoDiscount}% OFF</small>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <span className="cash-price">AED {(item.price * (item.qty || 1)).toFixed(2)}</span><br />
                      <strong className="crypto-price">
                        AED {((item.price * (item.qty || 1)) * (1 - (item.cryptoDiscount || 10) / 100)).toFixed(2)}
                      </strong>
                    </div>
                  </li>
                ))
