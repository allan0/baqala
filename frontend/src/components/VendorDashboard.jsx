...
// VERSION 21 (STABLE Hub RESTORATION)
// ================================================
...
export default function VendorDashboard({ user, lang }) {
  ...
  // 1. Merchant Sync: Fetch store linked to profile UUID
  const fetchMerchantHub = async () => {
    ...
    try {
      ...
      console.warn("Store not registered for this identity.");
      setMyBaqala(null);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // Pick up new 'merchant' role globally
      window.location.reload();
    } catch (e) {
      alert("Name is taken. Please choose another for your Hub.");
    } finally {
  ...
  const handleAddItem = async (e) => {
    ...
    } catch (e) { alert("Server error while listing item."); }
  ...
  if (!myBaqala) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-10 pt-20 text-center">
      <div className="w-24 h-24 bg-teal-400/10 rounded-full flex items-center justify-center mx-auto mb-10 shadow-2xl border border-teal-400/20">
      ...
      <form onSubmit={handleRegister} className="space-y-5 text-left">
        <div>
          <label className="text-[10px] font-black uppercase text-white/30 ml-4 tracking-[2px] block mb-2">{t.shop_name}</label>
        ...
        <div>
          <label className="text-[10px] font-black uppercase text-white/30 ml-4 tracking-[2px] block mb-2">{t.payout_wallet}</label>
          <input className="input-modern w-full !rounded-full !py-5" placeholder="TON Wallet Address" value={regForm.wallet} onChange={e => setRegForm({...regForm, wallet: e.target.value})} />
        </div>
        <button type="submit" disabled={isActionLoading} className="btn-primary w-full !py-6 mt-8 !rounded-full text-xl shadow-xl shadow-teal-400/20">
  ...
  return (
    <div className={`px-5 pt-4 ${isRTL ? 'text-right' : 'text-left'}`}>
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="stats" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div className="glass-card !p-8 shadow-2xl rounded-[40px]">
               <div className="flex justify-between items-center mb-10 px-1">
                  <h3 className="text-[11px] font-black uppercase tracking-[4px] text-white/30">{t.trusted_neighbors}</h3>
               ...
               <div className="flex -space-x-5 overflow-hidden pb-2">
                  {residents.filter(r=>r.status==='approved').length === 0 ? (
                    ...
                    residents.filter(r=>r.status==='approved').map((r, i) => (
                        <div key={i} className="w-16 h-16 rounded-full border-4 border-[#0a0a0f] bg-teal-400 flex items-center justify-center text-black font-black shadow-2xl uppercase text-lg border-2 border-teal-400/50 hover:translate-y-[-10px] transition-transform overflow-hidden">
          ...
          <motion.div key="catalog" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 pb-20">
             <div className="glass-card !p-8 shadow-xl border-white/5 rounded-[40px]">
                ...
                <form onSubmit={handleAddItem} className="space-y-5">
                   <input className="input-modern w-full !rounded-3xl shadow-inner" placeholder={t.item_placeholder} value={itemForm.name} onChange={e=>setItemForm({...itemForm, name: e.target.value})} required />
                   <div className="flex gap-4">
                      <input className="input-modern flex-1 !rounded-3xl shadow-inner" type="number" step="0.01" placeholder={t.price_label} value={itemForm.price} onChange={e=>setItemForm({...itemForm, price: e.target.value})} required />
             ...
             <div className="grid grid-cols-2 gap-6">
                {inventory.map(item => (
                  <div key={item.id} className="glass-card !p-5 relative group hover:border-teal-400/30 transition-all rounded-[32px] bg-white/[0.01] shadow-inner">
                     <div className="w-full aspect-square bg-black/40 rounded-full mb-6 flex items-center justify-center text-4xl shadow-inner border border-white/5 group-hover:scale-105 transition-transform duration-500">📦</div>
                     <h5 className="font-black text-[13px] leading-tight mb-3 truncate italic text-white/95 px-1">{item.name}</h5>
        ...
        {/* NEIGHBORS Hub (CRM & Approval) */}
        {activeTab === 'residents' && (
          <motion.div key="residents" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 pb-24">
             {residents.length === 0 ? (
               <div className="py-24 text-center opacity-20 italic text-xs font-black uppercase tracking-[10px]">{t.no_residents}</div>
             ) : (
               residents.map(r => (
                    ...
                    <div className="flex justify-between items-start mb-10 px-1">
                       <div className="flex items-center gap-5">
                          ...
                          <div>
                             ...
                             <p className="text-[10px] text-white/30 font-bold uppercase tracking-[2px]">Resident ID</p>
                          </div>
                       </div>
                       <span className={`text-[9px] font-black uppercase px-3.5 py-1.5 rounded-full ${r.status === 'approved' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 shadow-inner' : 'bg-orange-500/10 text-orange-400 border border-orange-500/20 animate-pulse'}`}>
          ...
          <motion.div key="shop" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 pb-24">
             <div className="glass-card !p-10 space-y-10 shadow-2xl rounded-[50px]">
                <div className="flex items-center gap-6 mb-2">
                    ...
                    <h3 className="text-[13px] font-black uppercase tracking-[5px] text-white/30 italic leading-none">Hub Identity Center</h3>
                </div>
                
                <div className="space-y-4 px-2">
                   <p className="text-[10px] font-black uppercase text-white/20 ml-5 tracking-[4px] leading-none">{t.shop_name}</p>
                ...
                <div className="space-y-4 px-2">
                   <p className="text-[10px] font-black uppercase text-white/20 ml-5 tracking-[4px] leading-none">{t.payout_wallet}</p>
                   <input className="input-modern w-full !rounded-full !py-6 font-mono text-[11px] !bg-black/40 shadow-inner text-white/80 border-white/5" value={myBaqala.wallet_address || 'UNSET'} readOnly />
                </div>
                ...
                <div className="p-8 bg-[#1a150e] border border-orange-500/20 rounded-[40px] flex gap-6 items-start shadow-inner">
                   ...
                   <p className="text-xs text-white/60 leading-loose font-medium uppercase tracking-widest italic">
                      Core store metadata (GPS Coordinates, Settlement Wallet) is verified by neighborhood consensus. Contact Grid Support for manual overrides.
                   </p>
                </div>
             </div>
             <p className="text-center text-[10px] text-white/10 font-bold uppercase tracking-[15px] pt-12 italic opacity-30">BAQALA ID: {myBaqala.id}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
