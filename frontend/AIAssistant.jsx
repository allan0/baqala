...
// VERSION 18 (FULL Hub RESTORATION & AI SYNC)
// ================================================
...
const loc = {
  ...
  ar: {
    ...
    guest_msg: "يجب إثبات الهوية. اربط حسابك من صفحة 'ملفي' لتفعيل الطلب بالصوت.",
...
export default function AIAssistant({ user, lang, currentBaqala, setActiveTab, onOrderSuccess }) {
  ...
  // Identity Guard: Determine if user is Guest or Linked
  ...
  const handleAIProcess = async () => {
    ...
    try {
      // --- 2. AI PARSING (Match voice/text to REAL inventory) ---
      ...
      if (res.data.success && res.data.items.length > 0) {
        // --- 3. Hub CHECKOUT (Commit matched items to DB Ledger) ---
        ...
        if (checkoutRes.data.success) {
          ...
          setTimeout(() => {
            ...
            setStatus('idle');
            setInput('');
            if (onOrderSuccess) onOrderSuccess(); // Trigger dashboard refresh
          }, 2500);
        }
      } else {
        throw new Error("No inventory match found");
      }
    } catch (e) {
  ...
  return (
    <>
      {/* Floating Toggle Button (CIRCULAR & SHIMMERING RESTORATION) */}
      <AnimatePresence>
        {!isOpen && (
          <motion.button 
            ...
            className="fixed bottom-32 right-6 w-16 h-16 rounded-full bg-gradient-to-tr from-teal-400 to-orange-500 text-black shadow-[0_20px_50px_rgba(0,245,212,0.4)] z-[45] flex items-center justify-center border-4 border-[#0a0a0f] active:scale-90 transition-transform"
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {/* Header: Shimmering Brand Restoration */}
            ...
            {/* Hub Context Awareness Card */}
            <div className="glass-card !bg-white/[0.02] mb-12 border-white/5 flex items-center gap-6 py-8 shadow-inner rounded-[40px] px-8">
               ...
               <div>
                  ...
                  <h3 className="font-black italic text-2xl text-white tracking-tighter leading-tight uppercase line-clamp-1">{currentBaqala?.name || t.nearest}</h3>
               </div>
            </div>

            {/* Intent Input & Visual Feedback Area */}
            <div className="flex-1 relative">
               ...
               {/* OVERLAY STATUS STATES Restoration */}
               <AnimatePresence>
                 {status === 'processing' && (
                 ...
                 {status === 'success' && (
                   <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="absolute inset-0 bg-[#0a0a0f]/90 backdrop-blur-2xl flex flex-col items-center justify-center gap-8 text-emerald-400 rounded-[50px] border border-emerald-500/20">
                      <div className="w-28 h-28 rounded-full bg-emerald-500/10 flex items-center justify-center border-4 border-emerald-500/20 shadow-[0_0_50px_rgba(16,185,129,0.2)]">
                 ...
                 {status === 'error' && (
                   <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 bg-[#0a0a0f]/90 backdrop-blur-2xl flex flex-col items-center justify-center gap-8 text-orange-500 rounded-[50px] p-12 text-center border border-orange-500/20">
                 ...
                 {status === 'guest_locked' && (
                   <motion.div 
                     ...
                     className="absolute inset-0 bg-[#0a0a0f]/98 backdrop-blur-3xl flex flex-col items-center justify-center p-12 text-center gap-10 rounded-[50px] border border-orange-500/30 shadow-2xl"
                   >
                      <div className="w-28 h-28 bg-orange-500/10 rounded-full flex items-center justify-center text-orange-500 shadow-2xl border border-orange-500/20 ring-8 ring-orange-500/5">
                        <UserCircle size={56}/>
                      </div>
                      ...
                      <button 
                        ...
                        className="btn-primary w-full !py-6 flex items-center justify-center gap-5 shadow-[0_30px_60px_rgba(0,245,212,0.3)] !rounded-full group"
                      >
            ...
            <div className="pb-20">
              ...
              >
                <span className="uppercase italic font-black text-3xl tracking-tighter group-active:skew-x-12 transition-all">{t.send_intent}</span>
